...
def read_racoro(filename, campaign='acapex'):
    '''Read a airborne 2DS & CAS combined Cloud Probe File into a DropSizeDistribution Object.
    Takes a filename pointing to a 2DS Horizontal & CAS G1 Aircraft file and returns a `DropSizeDistribution` object.
    Parameters:
    -----------
    filename: string
       2DS/CAS Cloud Probe Filename
    campaign: optional, string
        Optional campaign setting, currently does nothing. Defaults to acapex
    Usage:
    ------
    dsd = read_racoro(filename, campaign='acapex')
    ...
    reader = RacoroReader(filename, campaign)
...
class RacoroReader(object):
    ''' RacoroReader class for 2DS Cloud Probe Data.
    This class reads and parses data from 2DS/CAS combined data files. Use the read_racoro() function to interface with this.
    '''

    def __init__(self, filename, campaign='acapex'): 
        ''' Initializer for a 2DS/CAS combined Cloud Probe class.
            Read and process a 2DS/CAS combined Cloud Probe data file. This should only be called by the read_racoro() function.
            Parameters:
            -----------
            filename: string
